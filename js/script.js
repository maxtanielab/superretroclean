...
const urlInitialMap =
	...
	urlDepartments =
		"https://raw.githubusercontent.com/maxtanielab/repo-testing/main/departement.geojson";
		...
		if (
			...
			selectData.map((datas) => {
				...
				outputsData += `
						...
						<i class="qualification">REEE${qualification}</i>
			...
			function onEachFeature(feature, layer) {
				...
				if (feature.geometry.type === "Point") {
					...
					feature.properties.isActived
						...
						: "";
					feature.properties.isActived ? layer.on("click") : layer.off("click");
				}
					...
					if (
						...
						for (var i = 0; i < vals.length; i++) {
							...
							output += `
								<div class="map-item">
								<a href="${url}" class="title target="_blank">${shop} (${newShop})</a><br/>
								<i class="qualification">eeeeeeeeeee${qualification}</i>
								<p class="adress">${adress}</p>
								<a href="${url}" target="_blank" class="see-shop"> > Voir fiche info </a>
							  </div>`;
